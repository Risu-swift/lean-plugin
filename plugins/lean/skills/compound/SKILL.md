...
3. **For each one:**
   - Run `zk new --type decision|gotcha|pattern|fact --title "<the claim as a sentence>" --tags <up to 4, kebab-case> --source "<T-id, S-id or commit>" --body "Why: ...\nApply: ..."`. It checks both vaults for a similar note first.
   - If it prints `similar note exists`, Edit the file it names (sharpen the claim, add a case, add a tag) instead.
   - If that note is now wrong (a reversed decision, a fixed gotcha), rerun with `--force`, then `zk supersede <old> <new>` so search stops returning the old one.
...
4. **Note rules.**
   ...
   - The body is 10 lines or less and has an `Apply:` line; search results show that line, so it must stand alone.
